...
function widget
  if not set -q argv[1]
    ...
    echo
    echo 'Update widget on device from which this function was installed with a number of content parameters that can be string, progress, icon,     target or color.'
    echo
    ...
    echo
    echo 'Target is used to send different content to different widgets after configuring the widgets with different target identifiers which     requires echo 'the pro unlock. The target parameter is never assumed unless --target is used and is effective until next --target     parameter allowing echo 'updates of several widgets with a single command'
    echo
    echo 'String is the fallback type if nothing else matches, but content type can be forced for next parameter with --progress, --icon, --color,     echo '--text or --target with something like:'
    echo '  widget --text "50/100"'
    echo
    echo 'You can update several widgets at once by using --target to send all parameters until the next --target to a particular widget. Updating echo 'several widgets at once allows more total updates per day.'
  else
